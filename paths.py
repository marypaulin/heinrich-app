...
def create_output_dir(project_number: str):
    ...
    project_output_dir = OUTPUT_DIR / project_number
    project_output_dir.mkdir(parents=True, exist_ok=True)


def get_rechnung_template_path(project_number: str) -> Path:
    """Get the path to the Rechnung Word template."""
    path = OUTPUT_DIR / project_number / f"Vordruck_Rechnung_{project_number}.docx"
    return path
...
def get_output_paths(project_number: str, mode: str, order_number: str = None) -> Dict[str, Path]:
    ...
    if mode == 'liefer':
        liefer_path = OUTPUT_DIR / project_number / f"Lieferschein Nr. {project_number}.docx"
        return {'liefer': liefer_path}
    elif mode == 'rechnung':
        if not order_number:
            raise ValueError("ORDER_NUMBER is required for 'rechnung' mode.")
        auftrag_path = OUTPUT_DIR / project_number / f"Auftragsbestaetigung Nr. {project_number} - {order_number}.docx"
        rechnung_path = OUTPUT_DIR / project_number / f"Rechnung Nr. {project_number} - {order_number}.docx"
        return {
...
def clean_up_template(rechnung_template_path: Path):
    ...
    if rechnung_template_path.exists():
        ...
        logging.warning(f"Template to delete not found: {rechnung_template_path}")
